---
{"publish":true,"title":"Data Drift Detection","created":"2023-02-26","tags":["ai-ml"],"cssclasses":""}
---
...
- 그냥 다시 학습시키면 되는 거 아니야?
	- 가능한 해결책 중 하나이긴 하지만 모델 학습이나 데이터 획득에 큰 비용이 드는 경우, 시간이 지났다는 이유만으로 그 리소스를 들여서 무조건 재학습을 하는 것은 비효율적
	- 모델과 상호작용하여 데이터의 변화가 일어났다면, 그 변화의 양상을 전혀 모르는 채로 단순히 처음 모델링을 할 때의 프로세스를 처음부터 다시 거치는 것만으로는 문제가 해결되지 않을 수 있음
...
1. **개념적인 구분**
    ...
    - 이걸 쪼개보면,
        - $P(X)$의 변화 ( Covariate drift, Data drift ) : 인풋 피쳐의 분포가 변하는 경우
            ...
            - ![[assets/data drift detection/concept-drift-ex.png|325]]
        ...
        - $P(y|X)$ 의 변화 (Concept drift, Conditional change, Posterior class shift) : 인풋과 타겟 변수의 관계가 변하는 경우 - 가장 발견하고 규명하기 어려운 종류의 변화 
	        - ![[assets/data drift detection/concept-drift-ex2.png|283]] [이미지 출처](https://towardsdatascience.com/data-drift-explainability-interpretable-shift-detection-with-nannyml-83421319d05f)
            
    - 이런 것들을 개념적으로 쪼개 보는 이유는 다음과 같이 데이터의 변화가 반드시 모델의 결정경계를 건드리지는 않는 경우도 있기 때문!
	    - ![[assets/data drift detection/concept-drift-ex3.png|350]] [이미지 출처](https://dl.acm.org/doi/10.1145/2523813)
	    
        
2. **양상에 따른 구분**
    - 다음 그림을 보면 쉽게 이해됨
![[assets/data drift detection/concept-drift-classification.png|400]]
[이미지 출처](https://www.analyticsvidhya.com/blog/2021/10/mlops-and-the-importance-of-data-drift-detection/)
...
- 기본적으로는 비교 대상인 분포 2개의 데이터가 있어야 함
	- Tested distribution : 내가 비교하려는 현재의 분포 (보통 더 최신의 데이터)
	- Reference distribution : 그게 뭐에 비해서 달라진 것이냐? 를 보는 근거 분포 (이전 데이터거나 학습에 사용했던 데이터)
- sliding window로 일정 시간 텀마다 비교한다고 할 때 그 주기를 정하는 것도 중요함 (ex. N분 전, 어제, 전주, 전달)
	-  타이밍을 줄일수록 빠르게 drift를 발견할 수 있겠지만 노이즈도 많아지기 때문
	- 이 가게에 원래 금요일 밤에는 30대 직장인이 많이 방문하는데, 어제(목요일 밤)과 분포가 다르다고 문제가 생겼다고 볼 수는 없다
- 주기가 정해졌다면 drift의 정도를 하나의 단일한 값으로 나타내고 그 값에 대해 임계값을 정해야 함


### 분포 간 통계적 차이를 계산해보자

- 가장 간단한 접근으로, 각각의 피쳐에 대해 단변량으로 접근해서 하나하나 모니터링할 수 있음 

- **Kolmogorov-Smirnov Test (K-S Test)**
    - 보통 정규성 검정 방법으로 많이 알려져 있지만 원하는 분포-정규분포(1-sample) 간 검정 뿐 아니라 2개의 데이터가 같은 분포에서 나왔는지도 검정이 가능함(2-sample)
	    - ![[assets/data drift detection/ks-test.png|225]] [이미지 출처](https://en.wikipedia.org/wiki/Kolmogorov–Smirnov_test)
...
- **PSI (population stability Index)**
    - $\text{PSI} = \sum((\text{Tested} - \text{Reference}) \times \ln (\frac{\text{Tested}}{\text{Reference}}))$
        - 간단한 예시 - 지난주에는 페퍼로니 피자 주문 고객이 전체의 60% 였는데 이번주는 30% 라고 한다면 위 공식에 tested = 30%, reference = 60%로 계산해볼 수 있고 이걸 모든 메뉴에 대해 계산해서 합하면 됨
	        - ![[assets/data drift detection/pis.png|350]] [이미지 출처](https://mwburke.github.io/data%20science/2018/04/29/population-stability-index.html)
...
- 단변량 접근 말고 통째로 모델 단위로 접근하는 방법
	- 시간의 흐름에 따라 모델의 틀림 자체를 모니터링
	- 다음 방법들은 [scikit-multiflow](https://scikit-multiflow.readthedocs.io/en/stable/) 로 쉽게 써볼 수 있음


![[assets/data drift detection/monitoring-error.png|400]]
[이미지 출처](https://www.aporia.com/learn/data-drift/concept-drift-detection-methods/)
...
✅ 이렇게 모델 단위로 drift를 발견해도, 결국 뭐가 어떻게 문제인지 알아내고 싶다면 각각의 피쳐로 drill down해서 보고 분포의 차이를 명확히 검증해 봐야 하는 경우가 많을 것 같다

### 차원 축소 후 데이터를 복구해보자

- 데이터셋을 PCA 등으로 차원 축소한 다음 복구했을 때 데이터가 어떻게든 많이 바뀌었다면(PCA가 배운 분산의 형태에서 많이 떨어지게 되었다면) 복구 데이터의 오차가 커질 것이라고 가정
	- 원래 데이터포인트와 복구된 데이터 간의 유클리디언 거리를 drift 점수로 쓰는 것

- [NannyML](https://github.com/NannyML/nannyml) 패키지 공식 문서에서 PCA를 사용한 방법으로 다변량 drift를 발견한 예시가 있음

![[assets/data drift detection/butterfly-scatterplot.png|400]]

- 별로 현실적인 예시는 아니지만 이런 식으로 두 개의 피쳐의 관계가 휙 돌아가(?)버렸다면 분명 drift가 일어난 건 맞는데, 위와 우측에 그려진 각각의 단변량 분포를 보면 각각의 피쳐 내에서는 분포가 바뀐 것처럼 보이지가 않음 
	- 이럴 때 앞서 언급한 PCA 방법을 쓰면, 다음과 같이 복구 오차가 증가

![[assets/data drift detection/butterfly-multivariate-drift.png|400]]
