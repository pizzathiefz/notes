---
{"publish":true,"title":"고유값분해 (그리고 PCA)","created":"2023-12-24","tags":["linear-algebra"],"cssclasses":""}
---


>  [개발자를 위한 실전 선형대수학](https://m.hanbit.co.kr/store/books/book_view.html?p_code=B4279863215) 을 기반으로 이것저것 덧붙여서 작성함
...
- 이 식은 물론 $A$가 $\lambda$와 같다는 말이 아니고, 같은 벡터에 대해 이 행렬이 미치는 영향과 스칼라 값이 미치는 효과가 동일하다는 뜻임
- 기하학적으로는(그림으로 보자는 뜻) 행렬이 **벡터를 늘리거나 줄이긴 하지만 회전시키지 않는 경우**를 의미

![[assets/고유값분해 (그리고 pca)/vectors-example1.png|250]]
방향'이 다른 벡터들 ([그림 출처](https://www.coranac.com/documents/geomatrix/))

![[assets/고유값분해 (그리고 pca)/vectors-example2.png|250]]
2u는 u에다가 상수값을 곱한 것으로 u와 방향은 같으나 '크기'가 다르다 ([그림 출처](https://www.coranac.com/documents/geomatrix/))


![[assets/고유값분해 (그리고 pca)/vector-and-matrix-meets-1.png|348]]
([그림 출처](https://vitalflux.com/why-when-use-eigenvalue-eigenvector/))
많은 경우, 벡터에 행렬을 곱하면 방향이 바뀜


![[assets/고유값분해 (그리고 pca)/vector-and-matrix-meets-2.png|400]]
([그림 출처](https://vitalflux.com/why-when-use-eigenvalue-eigenvector/))
- 근데 어떤 특별한 행렬과 특별한 벡터가 만나면, 신기하게도 방향은 안 바뀌고 크기만 변할 수도 있다!
- 이때 이 벡터가 이 행렬의 고유벡터이며, 벡터가 늘어나거나 줄어드는 양이 고유값
예를 들면,
...
- 고유값과 고유벡터는 특정 행렬에 따라오는 존재임 -> 정해진 것이라기보단, 매칭 관계. 단 1:1 매칭은 아님
- **정의 상 고유값과 고유벡터를 얻으려면 행렬 $A$ 가 정방행렬(행과 열의 수가 같은 정사각형 행렬)이어야 함**
	- 벡터 $\mathbf{v}$ 가 $N$ 차원이라고 쳤을 때 $A$가 $N \times N$ 이어야 $A \mathbf{v}$ 와 $\lambda \mathbf{v}$가 같은 차원이 되지, $A$ 가 $M \times N (M \neq N)$ 이면 $A \mathbf{v}$ 는 $M$ 차원이 되어버려서 애초에 저 식이 성립할 수 없음

<br>

### 하는 법

- 고유값분해 주어진 행렬에 대해 고유값과 고유벡터를 찾는 것


$$ A\mathbf{v} - \lambda \mathbf{v} = (A- \lambda I) \mathbf{v} =  0 $$ 

- 행렬에서 스칼라를 빼줄 수는 없으니까 $A$랑 사이즈가 같은 단위행렬이라는 애를 불러와서 빼줌
- 단 행렬은 어떤 행렬을 곱해도 동일한 행렬이 된다는 점에서 상수 계산의 숫자 1이랑 비슷한 친구임.
- 이렇게 하게 되면 사실상 대각 성분에 스칼라 값만큼을 더하거나 빼주는 것인데, 이걸 행렬 이동이라고도 부름, 예를 들면


$$ \begin{bmatrix} 2 & 2 \\ 1 & 3 \end{bmatrix} + 3 \begin{bmatrix} 1 & 0 \\ 0 & 1 \end{bmatrix} = \begin{bmatrix} 5 & 2 \\ 1 & 6 \end{bmatrix} $$

- 이동시킨 $A- \lambda I$ 를 $A'$ 라는 새로운 행렬로 생각합하자. 즉 $A ' \mathbf{v} = 0$ 인 걸 생각해보자
- 이렇게 되려면 물론 아주 쉬운 답은 $\mathbf{v}$ 가 영벡터(0으로만 이루어진 벡터)이면 되지만, 수학은 이런 답을 뻔하다, 자명하다(trivial)고 하며 의미 있는 답으로 쳐주지 않음
- 이 식을 가능하게 하는 자명하지 않은 답은, **벡터 $\mathbf{v}$ 가 영벡터가 아니면서 행렬 $A'$의 영공간에 있다** 는 것
...
- **행렬의 공간**
	...
	- 물론 모든 행렬의 열공간이 이렇지는 않다. 
		...
		- ![[assets/고유값분해 (그리고 pca)/column-space.png|450]]
		- [(그림 출처)](https://angeloyeo.github.io/2020/11/17/four_fundamental_subspaces.html)
- **행렬의 계수(rank)**
...
- 다시 돌아와서 $A'$ 의 영공간의 벡터 $\mathbf{v}$를 찾아야 한다... 
- $A'$의 영공간이 비어있지 않으려면, $A'$는 선형종속 벡터 집합, 즉 rank-deficient한 singular 행렬이어야 하며, 행렬식의 값은 0임
...
- 이 이차 방정식을 풀면 2개의 $\lambda$ 값이 나옴 
- 즉 $2 \times 2$ 행렬에서 우리가 찾는 $\lambda$ 는  2개이며, 이는 행렬 크기가 올라가도 일반화될 수 있음 =  결론적으로 **$M \times M$ 행렬에 대해서는 $M$ 개의 고유값이 존재**

<br>
- 이제 고유벡터로 넘어가자
- 맨 처음 예시로 돌아가서, 다음 행렬의 고유값 2개 중 하나는 3

$$ \begin{bmatrix} 1 & 2 \\ 2 & 1 \end{bmatrix} \begin{bmatrix} 1 \\ 1 \end{bmatrix} = 3 \begin{bmatrix} 1 \\ 1 \end{bmatrix}  $$

- 그렇다는 건 요 행렬을 -3 이동시킨 $A'$ 의 영공간의  벡터가 고유벡터라는 건데,

$$\begin{bmatrix} -2 & 2 \\ 2 & -2 \end{bmatrix} \mathbf{v} = \begin{bmatrix} 0 \\ 0 \end{bmatrix}  $$
-  $\mathbf{v}$ 는 $\begin{bmatrix} 1 & 1 \end{bmatrix}$ 도 되고, $\begin{bmatrix} -3.1 & -3.1 \end{bmatrix}$ 도 되고, $\begin{bmatrix} 526 & 526 \end{bmatrix}$ 도 된다. 즉, 모든 실수값 $\alpha$ 에 대해 $\alpha \begin{bmatrix} 1 & 1 \end{bmatrix}$ 가 고유벡터가 될 수 있음!
- 고유값의 개수는 정해져 있지만 **고유벡터는 무한 개가 존재할 수 있음**. 벡터의 크기가 아니라 방향이 중요함
- 보통 라이브러리 등을 사용해서 고유벡터를 구해달라고 하면 노름이 1인 정규화된 고유벡터를 구해주는데 이건 그냥 대표 친구를 하나 알려주는 것임 
	- 여기서 노름은 벡터의 크기를 뜻하며, 벡터의 모든 원소를 제곱해서 더한 것에 제곱근을 취해준 결과

$$ \Vert \mathbf{v} \Vert = \sqrt{\sum^n_i v_i^2} $$

- 이 값이 1인 걸 단위벡터라고 하는데, 모든 벡터는 노름으로 모든 원소를 나눠줌으로써 단위벡터로 만들 수 있음

<br>

###  필수 조건과 행렬 대각화

-  $M \times M$ 행렬에 대해서는 $M$ 개의 고유값이 존재하는데

즉

$$ \begin{matrix} A\mathbf{v_1} = \lambda_1 \mathbf{v_1} \\ \vdots \\ A\mathbf{v_M} = \lambda_M \mathbf{v_M} \end{matrix} $$

- 이 여러 개의 식은 행렬화하여 간단하게 표현할 수 있음
	- 2차원 예시 -  어떤 행렬 A에 대해 2개의 고유값과 각각에 해당하는 고유벡터가 있을 때, 이 2개의 고유벡터를 열 단위로 합친 행렬 $V$와 고유값을 대각 원소로 사용하는 행렬 $\Lambda$를 다음과 같이 적어주는 것
...
- 그러면 저 쩜쩜쩜( $\vdots$ ) 을 쓰지 않고 식을 다시 쓸 수 있고
...
$$ A = V\Lambda V^{-1}$$ 
- 여기서 어떤 정방행렬이 고유값분해될 수 있는가 없는가? 를 판가름하는 중요한 조건이 등장
- 이 식이 성립하려면 $V$ 은 역행렬을 가질 수 있어야 하고, 역행렬을 가질 수 있는 조건은, 행렬식의 값이 0이 아니어야 한다는 것
...
- 즉 그 행렬의 열벡터들이 선형 종속이 아닌 경우를 의미함 
	- **결론적으로 $A$는 맨 처음 언급한 것처럼 정방행렬일 뿐만 아니라 $M$개의 고유 벡터들이 선형 독립이어야 고유값분해될 수 있음**.

- 저렇게 두번째 식처럼 A를 다시 쓰는 것을 멋진 말로 대각화(**Diagonalization**)라고 부름
	- 참고) [행렬의 고유값과 고유벡터](https://velog.io/@lighthouse97/행렬의-고유값과-고유벡터)

<br>

## 그리고 PCA

- 우리에게 변수가 10개인, 즉 10개 차원의 데이터가 있고, 이를 1차원으로 줄이고 싶다고 치자 
- PCA의 축소는 간단히 말하면 선형변환이므로 10차원의 벡터 1개를 임의로 정해서 (축) 각 데이터포인트와 이 벡터를 내적해서 선형변환을 할 수 있음
...
- 2차원 -> 1차원 예시
![[assets/고유값분해 (그리고 pca)/pca.png|475]]

[(그림 출처)](https://bookdown.org/tpinto_home/Unsupervised-learning/principal-components-analysis.html)


- 중요한 문제: 이 축 벡터 $\mathbf{z}$를 어떻게 구해야 차원 축소를 잘한 것이냐
- 축소할 때 원 데이터의 정보가 최대한 날아가지 않도록 하는 게 목적이라는 걸 기억하면, 왼쪽 그림의 까만 점(투영 후 데이터)가 더 몰려 있는 것으로 보아 정보가 더 많이 손실되었다고 볼 수 있음
- 우리가 찾고 싶은 축은 오른쪽 그림처럼 차원 축소한 결과물의 **분산이 최대한 커지도록** 하는 축

$$ \text{Var}(X\mathbf{z}) = \Vert X \mathbf{z} \Vert ^2 = \mathbf{z}^T X^TX\mathbf{z}$$


- 주어진 $X$ 에 대해 위 식을 최대로 하는 $\mathbf{z}$ 는 뭘까? 
	- 사실 저걸 최대로 만들려면 $\mathbf{z}$의 크기 자체를 엄청나게 키우면 된다...가 답일 리 없음. 이 문제에는 단위벡터( $\mathbf{z}^T\mathbf{z} = 1$)이라는 추가적인 제약 조건이 붙는다.
	- 또 $X^TX$ 요 부분을 살펴보면, 이 행렬은 원 데이터의 공분산행렬 $C$. 즉 $\mathbf{z}^T\mathbf{z} = 1$이면서 $\mathbf{z}^TC\mathbf{z}$가 최대인 $\mathbf{z}$ 를 찾는 문제
- 이 제약 조건 하에서 라그랑주 승수법을 쓰면,
...
- 결론적으로 **우리가 찾는 축은 $X$의 공분산행렬 $C$의 고유벡터**임
- 10개의 고유값과 그에 따른 단위벡터인 고유벡터가 나올 텐데요, $C$가 대칭행렬이기 때문에 이 고유벡터들은 모두 직교함(orthogonal)
