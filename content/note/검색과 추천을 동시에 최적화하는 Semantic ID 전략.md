...
- **데이터셋**: MovieLens 25M 기반 Joint S\&R 데이터셋
  ...
  - _검색 popularity bias 제거를 위해 uniform 분포 사용 → 실제 서비스에서는 유사 popularity 분포면 더 유리할 것_
...
1. **Search-based**: `all-mpnet-base-v2` 기반 bi-encoder를 검색 데이터 $D\_S$로 fine-tuning
...
- **Fused\_concat**: $v\_i^{search}$와 $v\_i^{rec}$를 $\ell\_2$-normalize 후 concatenation
  - $v\_i^{concat} = \[v\_i^{search}; v\_i^{rec}]$
  - 차원 불균형 문제: bi-encoder(386d) > ENMF(256d) → 검색 임베딩이 더 큰 영향력
- **Fused\_SVD**: 두 임베딩을 truncated SVD로 차원을 $d$로 맞춘 후 element-wise 합산
  - $v\_i^{svd} = v\_i^{search} + v\_i^{rec}$
  - Fused\_concat 대비 추천 성능 개선, 검색 성능은 소폭 하락
- **Multi-task**: bi-encoder를 검색($D\_S$)과 추천($D\_R$)을 동시에 학습
...
- **Task-specific**:
  ...
  - _검색 fine-tuned IDs는 추천 성능 60% 하락, 반대도 동일한 패턴_

- **Cross-task**:
  - Separate → 태스크 간 지식 공유 없어 정규화 효과 소실
  - Prefix-share → quantization 품질 문제(인 것 같다고 함..)로 최하위
  - Fused\_concat → 차원 불균형으로 추천 성능 저하
  - Fused\_SVD: → SVD로 차원 맞추면 추천 회복, 검색 소폭 손실
...
- Fused\_concat에서 임베딩 차원 불균형(386 vs 256)이 성능에 크게 영향을 준다는 점은 실제 적용 시 중요한 포인트
