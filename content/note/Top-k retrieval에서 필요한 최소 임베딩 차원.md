...
- 쿼리 임베딩을 자유롭게 최적화하는 대신, **answer set의 centroid**로 고정: $w\_q = \frac{1}{|S|}\sum\_{x \in S} x$
...
$$k-1 \leq \text{MED}(m, k; \mathcal{F}\_{\ell\_2}) \leq 2k$$

- $\text{MED}(m, k; \mathcal{F}_{\ell\_2}) \leq \text{MED}(m, k; \mathcal{F}_{\text{linear}})$ (Proposition 3.3)
...
- **Probabilistic method** 사용: $v\_1, \ldots, v\_m \sim \mathcal{N}(0, I\_n/n)$을 랜덤 샘플링
- 두 벡터의 inner product 및 norm 집중 부등식:
  $$\Pr\left\[|\langle v\_i, v\_j \rangle| \geq \frac{1}{3k}\right] \leq 2\exp\left(-c\frac{n}{k^2}\right)$$
...
- 결과: MED-C는 **log-linear** 성장 (empirically $\approx 3.23 \log\_2 m$)
