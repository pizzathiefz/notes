...
$$
P(v, S' | u) = \sum\_{d \in {\Gamma, \Gamma'}} P(v, S' | d) P(d | u)
$$
...
$$
P(v, S' | d) = P(v | d) P(S' | d) = P(v | d) \prod\_{i \in S} (1 - P(i | d, S))
$$
...
- $P(v, S' | d)$를 쪼개면
  ...
  - $\prod\_{i \in S} (1 - P(i | d, S))$ 는 $S$에 이미 있는 아이템 $i$들이 카테고리 $d$를 얼마나 커버하고 있지 않은지
