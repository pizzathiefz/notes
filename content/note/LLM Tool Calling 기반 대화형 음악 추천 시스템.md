...
사용자 $u$, 프로필 $p_u$, 이전 대화 상태 $s_{t-1}$, 현재 쿼리 $q_t$가 주어졌을 때:

$$\mathcal{C}_t = \text{LLM}(q_t, s_{t-1}, p_u; \mathcal{P}_{tool}, \mathcal{T})$$

$$m_t = \text{ToolEnv}(\mathcal{C}_t; \mathcal{D})$$

$$r_t = \text{LLM}(m_t, q_t, s_{t-1}, p_u; \mathcal{P}_{response})$$
