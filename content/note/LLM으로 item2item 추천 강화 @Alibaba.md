...
- **학습 방식**: SFT (Supervised Fine-Tuning)
  - 입력: User feature (User ID, 구매 이력 등) + Prompt instruction $P\_g$
...
$$L\_g = \sum\_{u\_i \in U} \sum\_{t=1}^{T} w^{t+1}_{u\_i} \cdot \text{loss}(Y^{t+1}_{u\_i} | P\_g, (X^s\_{u\_i}, X^{h,t}\_{u\_i}); \Theta\_g)$$

- $X^s\_{u\_i}$: 정적 사용자 정보 (ID, 카테고리 등), $X^{h,t}\_{u\_i}$: 동적 히스토리 행동 기록
- **Long-tail loss**: long-tail item 생성을 장려하는 가중치 적용

$$w^{t+1}_{u\_i} = \begin{cases} \alpha & \text{if } Y^{t+1}_{u\_i} \text{ is long-tail item} \ \beta & \text{otherwise} \end{cases}$$
...
$$L\_d = \sum\_{u\_i \in U} \sum\_{Y\_k \in Y^+_{u\_i} \cup Y^-_{u\_i}} \text{loss}\left(\text{sgn}(u\_i, Y\_k) \mid P\_d, X\_{u\_i}, X\_{Y\_k}; \Theta\_d\right)$$

- $Y^+_{u\_i}$: 실제 상호작용 아이템 (positive), $Y^-_{u\_i}$: 랜덤 샘플링한 negative 아이템
