...
- UniTi (Unified User Timeline)
  ...
  - $\mathbf{m} = (e\_{1}, e\_{2}, \dots)$ 이 하나의 미디어를 표현하는 벡터(각 원소가 미디어 ID, 작성자 ID, topic Id등 미디어 속성값을 나타냄)라고 하면, 2가지 유형의 타임라인을 만들 수 있음
    - Explicit timeline(like, share등) $E\_e = \[..., (m\_i, \text{action\_type}\_i, t\_i), ...]$
    - Implicit timeline(미디어를 보거나 머무르는 행위) $E\_i = \[..., (m\_j, \text{video\_duration}\_j, \text{dwell\_time}\_j, t\_j), ...]$
- **Multi-Slicing**
  ...
  - Implicit timeline slicing
    ...
    - Duration-debiased Weighted Watch Score slicing
      ...
      - $s(E\_i) = \frac{\ln(\text{dwell\_time} + 1)}{\ln(\gamma \cdot \text{video\_duration} + 1)}$
  - Time slicing
...
- **Funnel Summarization Arch (FSA)**
  ...
  - Funnel Transformer Encoder (여러 블록으로 구성)
    ...
    - 결과:  $N\_{\text{out}}$ (예: 58)개의 $D$차원 임베딩으로 이루어진 더 짧은 시퀀스로 압축
...
- 다운스트림 모델
  - Ranking
    ...
    - $\text{NE}\_k = \frac{\text{BCEloss}(\hat{y}^{(k)}, y^{(k)}) - \[p\_k \ln(p\_k) + (1-p\_k) \ln(1-p\_k)]}{\[p\_k \ln(p\_k) + (1-p\_k) \ln(1-p\_k)]}$
  - Retrieval
