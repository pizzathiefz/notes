...
- **Non-habitual streaming 정의** :
    - Habitual show: 최근 28일 내 청취 시간 $T_{u,s} \geq 10$분
...
- **Residual K-Means (R-KMeans)** 양자화:
    - 레벨 $m$마다 잔차를 가장 가까운 centroid $\mu_{m,j}$로 양자화:
        - $r_0 = x_e$
        - $c_m = \arg\min_{j} \| r_{m-1} - \mu_{m,j} \|_2$
...
- 유저 임베딩 $v_u$를 2-layer MLP로 LLM hidden dimension으로 projection:
