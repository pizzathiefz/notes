...
- 여러 임베딩 소스를 agnostic하게 처리
  - 각 modality $x\_m$을 별도 encoder $\mathcal{E}\_m$으로 latent vector $z\_m$으로 인코딩
  - Concatenation $\tilde{z} = \[z\_1, \ldots, z\_M]$ 후 projection → 통합 feature vector $z$
...
- **Progressive Masking**
  ...
  - Binary mask $m\_l \in {0,1}$: $m\_l = \mathbf{1}\_{l < r}$, $r \in \[1, L]$ 은 random integer
  - 학습 시 SID의 처음 $r$개 level만 선택적으로 사용
  - Quantized vector: $\hat{z} = \sum\_{l=1}^{L} m\_l e\_l^\*$
...
$$\mathcal{L}_{con} = -\sum_{i=1}^{2N\_b} \frac{\exp(\text{sim}(p\_i, p\_i^+))}{\sum\_{j=1}^{2N\_b} \exp(\text{sim}(p\_i, p\_j))}$$

- $p\_i$: 배치 내 영상 표현, $p\_i^+$: 영상 $i$와 co-occur한 영상의 표현
...
$$\mathcal{L}_{SFT} = -\sum_{t=1}^{L} r(\text{user}, v\_{click}) \cdot \log P(\text{sid}_t | \text{Context}_\text{user}, \text{History}_\text{user}, \text{sid}_{\<t})$$

- $r(\text{user}, v\_{click})$: 클릭별 handcrafted reward signal
