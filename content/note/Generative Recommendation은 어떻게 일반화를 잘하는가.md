...
- **Item Transition**: $\lbrack i\_s \to i\_t \rbrack$, 유저 히스토리에서 $i\_s$에서 $i\_t$로의 방향성 있는 쌍
...
- **Transitivity**: 중간 item $x$를 통해 $\lbrack i\_{t-1} \to x \rbrack$와 $\lbrack x \to i\_t \rbrack$가 각각 training에 존재 (다른 유저)
  - 두 개의 학습 예시를 조합해야 함
- **Symmetry**: 역방향 $\lbrack i\_t \to i\_{t-1} \rbrack$이 training에 존재
...
- **Substitutability**: $k \geq 2$인 k-hop transition $\lbrack i\_{t-k} \to \cdots \to i\_t \rbrack$가 training에 존재
