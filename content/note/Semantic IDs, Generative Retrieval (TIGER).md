...
- 사용자의 아이템 소비 기록 (item 1, ..., item n)을 semantic ID 토큰 시퀀스로 변환 
	- $(c_{1,0}, \cdots, c_{1,m-1}, \cdots, c_{n_{0},}\cdots,c_{n,m-1})$
- 모델은 이 시퀀스를 입력받아 사용자가 다음으로 상호작용할 item n+1의 semantic ID $(c_{n+1, 0}, \cdots, c_{n+1, m-1})$ 를 예측하도록 학습됨
...
- new capabilities
	...
	- 추천 다양성: 디코딩 시 temperature-based sampling을 통해 다양성을 조절할 수 있는데([[wiki/생성 모델의 Temperature]]), 예측된 top-k item의 실제 카테고리 분포에 대한 entropy@K를 확인한 결과 temperature를 높일수록 다양성을 향상시킬 수 있음
